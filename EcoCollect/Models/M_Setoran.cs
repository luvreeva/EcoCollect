...
namespace EcoCollect.Models
{
    public class SetoranModel
    {
        ...
        public int ExecuteSimpanSetoran()
        {
            using (var conn = DbConnection.GetConnection())
            {
                conn.Open();
                using (var transaction = conn.BeginTransaction())
                {
                    ...
                    {
                        string querySetor = @"
                            INSERT INTO transaksi_setor (kode_transaksi, id_nasabah, id_petugas, tanggal, total_nilai)
                            VALUES (@kode_transaksi, @id_nasabah, @id_petugas, @tanggal, @total_nilai)
                            RETURNING id_setor";

                        int idSetor;
                        using (var cmd = new NpgsqlCommand(querySetor, conn, transaction))
                        {
                            cmd.Parameters.AddWithValue("@kode_transaksi", this.KodeTransaksi);
                            cmd.Parameters.AddWithValue("@id_nasabah", this.IdNasabah);
                            cmd.Parameters.AddWithValue("@id_petugas", this.IdPetugas);
                            cmd.Parameters.AddWithValue("@tanggal", this.TanggalSetor);
                            cmd.Parameters.AddWithValue("@total_nilai", this.TotalNominal); 
                        ...
                        string queryDetail = @"
                            ...
                            VALUES (@id_setor, @id_kategori, @berat_kg, @harga_saat_transaksi, @subtotal)";

                        foreach (ItemSetoranModel item in this.DetailSetoran)
                        {
        ...
        public StrukSetoranModel GetStrukSetoran(int idSetor)
        {
            ...
            using (var conn = DbConnection.GetConnection())
            {
                conn.Open();
                string queryHeader = @"
                    SELECT ts.id_setor, ts.kode_transaksi, ts.tanggal, ts.total_nilai, n.nama_lengkap, n.no_hp
                ...
                if (struk == null) return null;

                string queryDetail = @"
                    SELECT ks.nama_jenis, ds.berat_kg, ds.harga_saat_transaksi, ds.subtotal
